...
by MirTrudMay®";
        ...
        private static string afterResult = @"Ну как вам? Будем рады отзыву о боте, они нам очень помогают☺️ Написать отзыв можно сюда: ";
        private static readonly TelegramBotClient Bot = new TelegramBotClient("");//id вашего бота
        ...
        {
            return id == 1111111111;//id администратора
        }
                            ...
                            Thread thread = new Thread(async () =>
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        while (true)
                                        {
                                            try
                                            {
                                                ...
                                                {
                                                    Bot.DeleteMessageAsync(message.Chat, message.MessageId);
                                                    await Bot.SendTextMessageAsync(e.Message.Chat, "Мне очень жаль, но возникли непредвиденные обстоятельства и я не смог справиться с заданием. Попробуйте ещё раз или напишите моему владельцу (@mirtrudsprite)");
                                                    return;
                                                }
                                    ...
                                    {
                                        Bot.DeleteMessageAsync(message.Chat, message.MessageId);
                                        await Bot.SendTextMessageAsync(e.Message.Chat, "Я не смог обработать вашу фотографию, пожалуйста, сообщите об этом моему владельцу (@mirtrudsprite). Я хочу стать лучше");
